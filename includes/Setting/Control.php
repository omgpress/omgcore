...
class Control extends Feature {
	...
	public static function render(
		...
		if ( in_array( $type, array_keys( static::$control_classes ), true ) ) {
			...
			$core->debugger()->die( "Undefined type <code>'$type'</code> of a setting control.$parent_label" );

			$control_classname = '';
		}

		if ( ! method_exists( $control_classname, 'render' ) ) {
			$core->debugger()->die( "Not found the static method <code>render</code> in the control class $control_classname." );
		...
		?>
		<tr class="wpt-control">
			<?php if ( $title_method_exists ) { ?>
				<th class="wpt-control__title" scope="row">
					<?php call_user_func( array( $control_classname, 'render_title' ), $type, $key, $title, $args, $required_label ); ?>
				</th>
			<?php } ?>
			<td class="wpt-control__body" <?php echo $title_method_exists ? '' : 'colspan="2"'; ?>>
