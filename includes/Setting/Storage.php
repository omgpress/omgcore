...
class Storage extends Feature {
	...
	public function add_tab( string $tab, string $page, string $nav_title, ?string $title, ?string $description ): void {
		$this->instances[ $page ]['children'][ $tab ] = array(
			'object'   => new Tab( $this->app, $this->core, $this, $tab, $page, $nav_title, $title, $description ),
			'children' => array(),
		);
	}
	...
	public function add_sub_tab( string $sub_tab, string $tab, string $page, string $nav_title, ?string $title, ?string $description ): void {
		$this->instances[ $page ]['children'][ $tab ]['children'][ $sub_tab ] = array(
			'object'   => new Sub_Tab( $this->app, $this->core, $this, $sub_tab, $tab, $page, $nav_title, $title, $description ),
			'children' => array(),
		);
	}
	...
	public function add_box( string $box, ?string $sub_tab, string $tab, string $page, ?string $title, ?string $description ): void {
		$data = array(
			'object'   => new Box( $this->app, $this->core, $this, $box, $sub_tab, $tab, $page, $title, $description ),
			'children' => array(),
		);
	...
	public function get_content(
		...
		if ( $box ) {
			if ( $sub_tab ) {
				return $this->instances[ $page ]['children'][ $tab ]['children'][ $sub_tab ]['children'][ $box ]['children'][ $content ] ?? null;

			} else {
				return $this->instances[ $page ]['children'][ $tab ]['children'][ $box ]['children'][ $content ] ?? null;
			}
		} elseif ( $sub_tab ) {
	...
	public function get_active_page( bool $raw = false ): ?string {
