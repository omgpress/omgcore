...
class Hook extends Feature {
	...
	public function add_action( string $slug, callable $callback, int $priority = PRIOR, int $accepted_args = 1 ): App {
	...
	public function activation( callable $callback ): App {
		if ( $this->is_theme() ) {
			add_action( 'after_switch_theme', $callback, PRIOR, 2 );
	...
	public function deactivation( callable $callback ): App {
		if ( $this->is_theme() ) {
			add_action( 'switch_theme', $callback, PRIOR, 3 );
